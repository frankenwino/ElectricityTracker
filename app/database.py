...
def latest_price_data():
    query = db.select(
        [
            day_ahead.columns.date,
            day_ahead.columns.start_hour,
            day_ahead.columns.end_hour,
            day_ahead.columns.sek_per_kwh,
        ]
    ).where(day_ahead.columns.date_start_hour >= current_date_start_hour())
